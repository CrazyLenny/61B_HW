package edu.berkeley.cs.cs61b.hw.hw3;
...
public class Homework3 {
   ...
   **/
	...
	public static void smoosh(int[] ints) {
		// If there are no duplicates, loop whole array
		// else if there are duplicates, then move duplicate to end and reduce
		// loop limit by 1
		int lim = ints.length;

		int index = 0;
		while (index < lim) {
			// check if there are duplicates, if so move to end, move all
			// elements 1 left and make limit down by 1
			if ((index + 1) < ints.length && ints[index] == ints[index + 1]) {
				for (int index2 = index + 1; index2 < ints.length - 1; index2++) {
					ints[index2] = ints[index2 + 1];
				}
				ints[ints.length - 1] = -1;
				lim--;
			} else {
				// increment index only when next element is not a duplicate
				index++;
			}
		}
	}
